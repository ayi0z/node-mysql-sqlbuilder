...
export type IAggrFnType = 'COUNT' | 'SUM' | 'AVG' | 'MAX' | 'MIN'
type WhereFieldType = { [index: string]: string | number | OperatorType }
export type WhereArgType = string
    | { [index in AorOperator]: WhereFieldType | WhereFieldType[] }
    | WhereFieldType

export interface IFieldFn { toSql(): string | undefined | [string, string] }
export interface IWhereOperator extends IFieldFn {
    cmd: OperatorType["operator"]
    field: string
    value: OperatorType["item"]
}
export interface DataType { [index: string]: number | string | bigint }
export interface IAggr { fun: IAggrFnType, field: string, disAll: DistAllType, alias?: string }
